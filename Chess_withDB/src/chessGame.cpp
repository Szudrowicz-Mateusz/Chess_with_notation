...
: board(bordCol1,bordCol2)
{
    ...
    whiteClockText.setCharacterSize(24);
    whiteClockText.setFillColor(sf::Color::White);
    whiteClockText.setPosition(530.f, 400.f); // Adjust the position as per your needs

    blackClockText.setFont(font);
    blackClockText.setCharacterSize(24);
    blackClockText.setFillColor(sf::Color::White);
    blackClockText.setPosition(530.f,430.f); // Adjust the position as per your n
...
void ChessGame::restart(){
    ...
    database.startGame(time,"*"); // Variable time is inicialized in database.h
...
void ChessGame::updateInfo(){
    ...
    // Update the clock texts
    whiteClockText.setString("White Time: " + std::to_string(white_time));
    blackClockText.setString("Black Time: " + std::to_string(black_time));
    ...
    if(!mate){
        ...
        if(playerTurnCheck)
        {
            ...
            database.mate(); // Just mate so we add '+'
        }
        ...
        if(playerTurnCheck){
            if(playerTurn){
                ...
                database.checkMate(true); // White wins
            }
            else{
                textSituation.setString("CHECKMATE\nWhite Wins");
                database.checkMate(false); // Black wins
            }
        }
        else{
            textSituation.setString("STALEMATE\nIts a DRAW");

            database.draw(); // A draw
        }
    }

    if(black_time <= 0){
        textSituation.setString("Times out\nWhite Wins");
        database.checkMate(false); // White wins
    }
    else if(white_time <= 0){
        textSituation.setString("Times out\nBlack Wins");
        database.checkMate(true); // Black wins
    }


    if(!playerTurn)
        database.flush(white_time);
    else
        database.flush(black_time);
}
...
void ChessGame::draw(sf::RenderTarget& target, sf::RenderStates states) const{
    ...
    target.draw(whiteClockText);
    target.draw(blackClockText);
...
void ChessGame::moveSelected(int pos){
    ...
    if(validMove){
        ...
        if((selectedPiece->getType() == 'K') && (!selectedPiece->getMoved())){
            if(selectedPiece->getPlayer()){ // If white
                ...
                if(pos == 62){
                    ...
                    database.castling(true); // Short castling
                }    
                else if(pos == 58){
                    whitePieces[0].setPosition(59);
                    database.castling(false); // Long castling
                }
        ...
        // Database
        database.insertMoves(lastMove , old_pos);
        ...
        for(int i=0; i<16; i++){
            if(selectedPiece->getPlayer()){ // If White
                if(blackPieces[i].getPosition() == pos){
                    ...
                    database.pieceIsTaken(); // Adding ":" because piece is taken by whites
                    break;
                }
            }
            else{ // If Black
                if(whitePieces[i].getPosition() == pos){
                    ...
                    database.pieceIsTaken(); // Adding ":" because piece is taken by blacks
                    break;
                }
            }
        }
        ...
        if (playerTurn)
            black_time -= static_cast<int>(elapsedSeconds);
        else
            white_time -= static_cast<int>(elapsedSeconds);
